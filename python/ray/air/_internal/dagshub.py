...
class _DagsHubLoggerUtil(_MLflowLoggerUtil):
    ...
    def _dvc_add(self, local_path="", remote_path:str = ""):
        if not os.path.isfile(local_path):
            raise FileNotFoundError(f"The file is not exist!, currently recieved path: {local_path}")
    ...
    def upload(self, run_id: str=""):
        self._dvc_commit(self.dvc_folder, commit=f"ray.io: Add all artifacts from run {run_id}")

    def save_artifacts(self, dir: str, run_id: Optional[str] = None, remote_path:str = "artifacts"):
